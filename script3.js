...
document.getElementById('comentariosForm3').addEventListener('submit', function(event) {
    ...
    localStorage.setItem('comentarios3', JSON.stringify(comentariosGuardados));

    // Actualizar la lista de comentarios mostrada en la página
    mostrarComentarios();

    // Limpiar el formulario después de enviar
    document.getElementById('comentariosForm3').reset();
});

// Función para mostrar los comentarios guardados en la página
function mostrarComentarios() {
    const listaComentarios = document.getElementById('lista-comentarios3');
    listaComentarios.innerHTML = ''; // Limpiar la lista de comentarios

    // Obtener los comentarios almacenados en localStorage
    let comentariosGuardados = JSON.parse(localStorage.getItem('comentarios3')) || [];
